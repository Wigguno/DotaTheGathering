...
function _ScoreboardUpdater_UpdatePlayerPanel( scoreboardConfig, playersContainer, playerId, localPlayerTeamId )
{
	...
	if ( playerInfo )
	{
		...
		var playerScoreTable = CustomNetTables.GetTableValue( "score_table", "player_" + playerId );
		if (playerScoreTable != null)
		{
			_ScoreboardUpdater_SetTextSafe( playerPanel, "Score", CustomNetTables.GetTableValue( "score_table", "player_" + playerId ).score );
			_ScoreboardUpdater_SetTextSafe( playerPanel, "Tier1", CustomNetTables.GetTableValue( "score_table", "player_" + playerId ).type1 );
			_ScoreboardUpdater_SetTextSafe( playerPanel, "Tier2", CustomNetTables.GetTableValue( "score_table", "player_" + playerId ).type2 );
			_ScoreboardUpdater_SetTextSafe( playerPanel, "Tier3", CustomNetTables.GetTableValue( "score_table", "player_" + playerId ).type3 );
			_ScoreboardUpdater_SetTextSafe( playerPanel, "Tier4", CustomNetTables.GetTableValue( "score_table", "player_" + playerId ).type4 );
			_ScoreboardUpdater_SetTextSafe( playerPanel, "Tier5", CustomNetTables.GetTableValue( "score_table", "player_" + playerId ).type5 );
		}
...
//	$.Msg( "_ScoreboardUpdater_UpdateTeamPanel: ", teamId );
	...
	//$.Msg("TeamID: ", teamId, ". Local player team: ", Game.GetLocalPlayerInfo().player_team_id);
	if (teamId != Game.GetLocalPlayerInfo().player_team_id && scoreboardConfig.displayEnemy == false)
		return;
...
function ScoreboardUpdater_InitializeScoreboard( scoreboardConfig, scoreboardPanel )
{
	...
	if ( typeof(scoreboardConfig.displayEnemy) === 'undefined')
	{
		// default to true
		scoreboardConfig.displayEnemy = false;
	}
